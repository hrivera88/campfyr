...
import { setActiveRoom, setRoomMembership } from "../../store/slice/chatRoomSlice";
...
const RoomListItem = ({ room, isExpanded, onToggle, onUserSelect }: RoomListItemProps) => {
  ...
  const dbMutation = useMutation({
    mutationFn: () => api.post('/api/direct/conversations', {
    ...
    <>
      ...
      <Dialog open={showJoinModal} onClose={() => setShowJoinModal(false)}>
        ...
        <DialogActions sx={{ pb: 2, px: 1.8 }}>
          <Button size="small" onClick={() => setShowJoinModal(false)}>
            Cancel
          </Button>
          <Button
            size="small"
      ...
      <Dialog open={showDMDialog} onClose={() => setShowDMDialog(false)}>
        ...
        <DialogActions sx={{ pb: 2, px: 1.8 }}>
          <Button size="small" onClick={() => setShowDMDialog(false)}>
            Cancel
          </Button>
          <Button
            size="small"
